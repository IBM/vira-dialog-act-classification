...
from service import get_model_predictions, model_trained_classes
...
def tune_threshold():
    df = pd.concat([pd.read_csv(os.path.join(DATASET_DIR, "validation.csv")), pd.read_csv(os.path.join(DATASET_DIR, "test.csv"))])
    candidates = df['sentence'].tolist()
    num_classes = len(model_trained_classes)
    ...
    y_scores = np.array([[score if i == model_trained_classes.index(pred_class) else 0 for i in range(num_classes)]
